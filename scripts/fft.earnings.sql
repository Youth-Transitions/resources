...
create table fft.earnings (
	edukey char(14) not null,
	year smallint not null,
	self_employed bit not null,
	...
	tax_year as cast(year-1 as varchar(16))+'/'+right(cast(year as varchar(4)), 2),
	primary key(fft_person_id, year, self_employed)
	);
else truncate table fft.earnings;
...
from
	...
	from
		(
		select edukey from leo_i2."<project number>".leo_earnings
		union all select edukey from leo_i2."<project number>".leo_self_assessment
		) _
	where
		edukey <> ''
...
select
	max(leo_edukey),
	left(src.taxyear, 4) + 1,
	0 self_employment,
	sum(earnings),
	fft_person_id
from
	leo_i2."<project number>".leo_earnings src
	left join fft.person_lookup p
		on	p.leo_edukey = src.edukey collate Latin1_General_BIN
where
	earnings > 0
	and fft_person_id is not null
group by
	fft_person_id,
	left(src.taxyear, 4)
;
...
select
	...
	left(src.taxyear, 4) + 1,
	1 self_employment,
	sum(earnings_self_employment),
	fft_person_id
from
	leo_i2."<project number>".leo_self_assessment src
	left join fft.person_lookup p
		on	p.leo_edukey = src.edukey collate Latin1_General_BIN
where
	earnings_self_employment > 0
	and fft_person_id is not null
group by
	fft_person_id,
	left(src.taxyear, 4)
